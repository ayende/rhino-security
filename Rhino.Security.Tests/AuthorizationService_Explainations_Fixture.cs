namespace Rhino.Security.Tests
{
	...
	public class AuthorizationService_Explainations_Fixture : DatabaseFixture
	{
		...
		public void ExplainWhyAllowedWhenAllowedPermissionWasDefinedWithDenyPermissionWithLowerLevel()
		{
			...
			AuthorizationInformation information = authorizationService.GetAuthorizationInformation(user, "/Account/Edit");
			string expected =
...
";
		...
		public void ExplainWhyNotAllowedIfDenyPermissionWasDefined()
		{
			...
			AuthorizationInformation information = authorizationService.GetAuthorizationInformation(user, "/Account/Edit");
			Assert.AreEqual("Permission (level 1) for operation '/Account/Edit' was denied to 'Ayende' on 'everything'\r\n", information.ToString());
		}
		...
		public void ExplainWhyAllowedIfAllowPermissionWasDefined()
		{
			...
			AuthorizationInformation information = authorizationService.GetAuthorizationInformation(user, "/Account/Edit");
			Assert.AreEqual("Permission (level 1) for operation '/Account/Edit' was granted to 'Ayende' on 'everything'\r\n", information.ToString());
		}

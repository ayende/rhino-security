namespace Rhino.Security
{
	...
	public class AuthorizationService : IAuthorizationService
	{
		...
		{
			Permission[] permissions = permissionsService.GetPermissionsFor<TEntity>(user, entity, operation);
		...
		private void AddPermissionDescriptionToAuthorizationInformation<TEntity>(string operation, AuthorizationInformation info, IUser user, Permission[] permissions, TEntity entity)
				where TEntity: class 
		{
			if (permissions.Length == 0)
			{
				...
				else
				{
					...
					info.AddDeny(Resources.PermissionForOperationNotGrantedToUserOnEntity,
						...
						user.SecurityInfo.Name, 
						Strings.Join(usersGroups),
						Security.GetDescription(entity),
						Strings.Join(entitiesGroups));
				}
				return;
			}
		...
		/// <param name="operation">The operation.</param>
		/// <returns></returns>
		public AuthorizationInformation GetAuthorizationInformation<TEntity>(IUser user, TEntity entity, string operation) where TEntity : class
		{
			AuthorizationInformation info;
			if (InitializeAuthorizationInfo(operation, out info))
				return info;
		...
		private static string GetPermissionTarget(Permission permission)
		{
			...
			if (permission.EntitySecurityKey != null)
				return Security.GetDescription(Type.GetType(permission.EntityTypeName), permission.EntitySecurityKey.Value);
